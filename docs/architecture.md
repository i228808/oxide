...
Oxide is a Cargo workspace with four crates:

```
Oxide/
├── Cargo.toml          # Workspace definition
├── oxide-framework-core/         # Framework library (the main crate)
├── oxide-framework-macros/       # Proc-macro crate (#[controller], route attributes)
├── oxide-framework-db/           # SQLx integration crate
└── oxide-framework-cli/          # `oxide` CLI — scaffold, codegen, run, test, bench
```

`oxide_framework_core` is the main library. `oxide_framework_macros` provides proc macros and is re-exported through `oxide_framework_core` so users only need one dependency.

## Module Map (`oxide_framework_core`)

```
oxide-framework-core/src/
...
The public API is deliberately flat. Users import from `oxide_framework_core` directly:
...
pub use axum::Json;
pub use oxide_framework_macros::controller; // #[controller("/prefix")] proc macro
```
...
```
Request → Logger → CORS → Timeout → RateLimit → CatchPanic → InjectState → JwtAuth → UserHooks → Router → Handler
```
...
2. **JwtAuth** — optional JWT/session validation; inserts `AuthClaims`
3. **InjectState** — makes `AppState` available to hooks and extractors
4. **CatchPanic** — converts panics in hooks/handlers into JSON 500 responses
5. **RateLimit** — per-IP fixed window; returns 429 with `Retry-After`
6. **Timeout** — returns JSON 408 if handler exceeds deadline
7. **CORS** — adds access-control headers to ALL responses (including errors)
8. **Logger** — logs method, path, status, and latency

## Current Scope

Implemented crates in this workspace today:

| Crate | Purpose |
|---|---|
| `oxide-framework-core` | Core runtime, builder, routing, middleware, auth, extractors |
| `oxide-framework-macros` | `#[controller]` proc macro and route attributes |
| `oxide-framework-cli` | `oxide` binary for scaffolding and code generation |
| `oxide-framework-db` | SQLx integration via `AppDbExt::database(...)` |
